...
# CSVデータを読み込み（商品名がなくても22000円は含める）
...
with open('決済-20250824-114821.csv', 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        # 商品名が見つかった場合、または22000円の場合は追加
        if product_found or amount == '22000':
            if amount == '22000' and not product_found:
                product = '無明会通常プラン'
                # 顧客名を他の列から探す
                card_name = row[38] if len(row) > 38 else ''
...
print(f'データ読み込み完了: {len(product_data)}商品, {total_customers}件')
print(f'成功: {success_count}件, 失敗: {failed_count}件, 返金: {refund_count}件')

# HTMLファイルを生成（データ埋め込み版）
